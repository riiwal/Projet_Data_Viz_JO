...
acp1 <- reactive({ dtaf_loaded_acp <-dtaf_loaded %>%
  select("acc_lycee","act_ouv" ,"acc_college","pop_urb","locatai","proprio","actdip_BAC3", # sélection des variables les mieux projetées pour l'acp
...
vars_vote <- reactive({c("Abs_insc","Vot_insc","Blanc_vote","Nul_vote",
                         ...
                         "Melenchon_exp","Hidalgo_exp","Jadot_exp",
                         "Pecresse_exp" ,"Poutou_exp","DupontAignan_exp") })

fviz_pca_ind(res.pca,
             geom.ind = "point",
             col.ind = dtaf_loaded$Gagnant,  # individus colorés par "vainqueur"
             palette = c("#223A77","#F2C14E","#D7263D"), # couleurs correspondants aux partis gagnants
...
acp2 <- reactive({ dtaf_loaded_acp2 <- dtaf_loaded %>% 
  select("acc_lycee","act_ouv" ,"acc_college","pop_urb","locatai","proprio","actdip_BAC3", # choix des variables
         "actdip_BAC3P","actdip_CAP","act_cad","actdip_BAC5","men_coupse") %>%
  st_drop_geometry()# on retire la géométrie de la carte
