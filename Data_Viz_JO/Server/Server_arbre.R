...
names(dtaf_clean) <- label_variable[names(dtaf_clean)]

# Construire l’arbre
res <- rpart(`Vote majoritaire` ~ ., data = dtaf_clean)
...
output$tree_cand <- renderVisNetwork({
  ...
  # Renommer les colonnes avec les labels
  names(dtaf_clean_cand) <- c(
    label_variable[names(dtaf_clean_cand)[names(dtaf_clean_cand) != input$cand]],
    input$cand)
  
  res_cand <- rpart(as.formula(paste(input$cand, "~ .")),data = dtaf_clean_cand)
